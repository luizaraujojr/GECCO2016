...
import unirio.experiments.multiobjective.model.ExperimentCycle;
import unirio.experiments.multiobjective.model.ExperimentInstance;
import unirio.experiments.multiobjective.model.ExperimentResult;
import unirio.experiments.multiobjective.model.ParetoFrontier;
import unirio.experiments.multiobjective.model.ParetoFrontierVertex;
...
public class ProjectExperimentFileReader extends ExperimentFileReader
{
	...
	private void readParetoFrontier(ParetoFrontier frontier, int objectiveCount, Scanner scanner) throws ExperimentFileReaderException
	{
		...
		while (s.length() > 0)
		{
			...
			if (position <= 0)
				throwException("expected overtime entry in Pareto front");
			
			double overtime = parseDouble(s.substring(0, position));
			s = s.substring(position+2);

			position = s.indexOf(";");
			
			if (position <= 0)
				throwException("expected duration entry in Pareto front");
			
			double duration = parseDouble(s.substring(0, position));
			s = s.substring(position+2);
			
			position = s.indexOf(";");
			
			if (position <= 0)
				throwException("expected cost entry in Pareto front");
			
			double cost = parseDouble(s.substring(0, position));
			s = s.substring(position+2);


			ParetoFrontierVertex vertex = new ParetoFrontierVertex(frontier.getObjectiveCount());
			vertex.set(0, overtime);
			vertex.set(1, duration);
			vertex.set(2, cost);
