...
#data <- subset(data, inst != "I3" | config != "CPM");
#data <- subset(data, inst != "I2" | config != "CPM");

for (instancia_ in instances)
{
	subdata <- subset(data, inst == instancia_);
	boxplot(best~config, data=subdata, main=instanceNames[which(instances==instancia_)], outline=FALSE);
}
