...
public class VerificaMetamodelo
{
	...
	private void verificaRepositorioTaxa (Metamodelo metamodelo, Classe classe, Comportamento rate) throws Exception
	{
		...
		{
			Relacionamento relacao = metamodelo.pegaRelacionamentoNome(rate.getRepositorio().getEsquerda().getNome());
	
			if (relacao == null)
				throw new Exception("The metamodel does not have the relation " + rate.getRepositorio().getEsquerda().getNome() + ", as required by the rate " + rate.getId());
	
			// Se o identificador for o nome do papel de destino ou nome da relação, retorna a classe destino
			String id = rate.getRepositorio().getEsquerda().getNome();
